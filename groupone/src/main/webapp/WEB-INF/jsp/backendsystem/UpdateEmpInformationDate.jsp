...
<meta charset="UTF-8">
<title>後臺系統</title>
<link rel="stylesheet" href="style/backend_system_style.css">
<style>
.gray1 {
	filter: grayscale(1);
}
...
<body>
	...
	<article class="block2">
		<div align="center">
			<h3 style="color: red">${emp.username}的個人資料</h3>
			<form method="post" action="${contextRoot}/updateEmpInformation"
				...
				<table>
					<tr>
						...
						<td><input type="text" name="empId" id="empId"
							size="10" value="${emp.empId}"  readonly="readonly">
						<td>姓名
						<td><input type="text" name="username"
							 size="10" value="${emp.username}">
						<td>出生年月日
						<td><input type="date" class="form-control" name="birthday"
							 size="10" value="${emp.birthday}">
					<tr>
						<td>部門
						<td><select class="form-select" aria-describedby="empDepart"
							name="fkDepartmentDeptn" id="fkDepartmentDeptn">
						...
						<td>職稱
						<td><select class="form-select" aria-describedby="empTitle"
							name="fkTitleId" id="fkTitleId">
						...
						<td>主管
						<td>
						<select class="form-select" aria-describedby="empSuperiorName"
							name="superiorName" id="superiorName" >
						</select>
					<tr>
						<td>身分證
						<td><input type="text" name="id" 
							size="10" value="${emp.id}">
						<td>手機
						<td><input type="text" name="phone" 
							size="10" value="${emp.phone}">
						<td>性別
						<td><input type="radio" id="male" name="contact" value="男" checked="true">
							<label for="contactChoice1">男</label> 
							<input type="radio" id="female" name="contact" value="女">
							<label for="contactChoice1">女</label>
					<tr>
						<td>最高學府
						<td><input type="text" name="highEdu" 
							size="10" value="${emp.highEdu}">
						<td>學歷
						<td><input type="text" name="highLevel"
							 size="10" value="${emp.highLevel}">
						<td>科系
						<td><input type="text" name="highMajor"
							 size="10" value="${emp.highMajor}">
					<tr>
						<td>緊急聯絡人
						<td><input type="text" name="emergencyContact"
							 size="10" value="${emp.emergencyContact}">
						<td>聯絡人關係
						<td><input type="text" name="contactRelationship"
							 size="10"
							value="${emp.contactRelationship}">
						<td>聯絡人手機
						<td><input type="text" name="contactPhone"
							 size="10" value="${emp.contactPhone}">
					<tr>
						<td>地址
						<td colspan="3"><input type="text" name="address"
							 size="30" value="${emp.address}">
						<td>email
						<td><input type="text" name="email"
							 size="30" value="${emp.email}">
				</table>
				<table>
				<tr>
				<td>狀態
				<td><select class="form-select" aria-describedby="empState"
							name="fkStateId" id="fkStateId">
	...
	$("#fkDepartmentDeptn").change(function(){
		let departId = $("#fkDepartmentDeptn").val();
		let empId = $("#empId").val();
		let titleId = $("#fkTitleId").val();
		$.ajax({
			url:'http://localhost:8080/GroupOne/api/getSupervisor?departId=' + departId + '&empId=' + empId + '&titleId=' + titleId,
			dataType:'json',
			method:'get',
			success:function(data){

				$('#superiorName option').remove()
				console.log(data.length);
				var supervisor = '';
				for(var i=0;i<data.length;i++){
					supervisor += '<option value="' + data[i].username + '">' + data[i].username + '</option>'
				}
				$('#superiorName').append(supervisor);
				console.log("HI");
			},
			error:function(err){
				console.log(err)
