								...
								<div class="card-body">
									<div>
										<form:form class="form"
											action="${contextRoot}/updatePercentages"
											modelAttribute="percentages" method="post"
											enctype="multipart/form-data" onsubmit="return checkSubit()">
											<form:input type="hidden" path="percentageId" />
											<div>
												<label>活動名稱:</label>
												<form:input path="percentageName" id="percentageName"
													onblur="checkNM()" />
												<span id="nmsp" style="font-size: 10px;"></span>
											</div>
											<div>
												<img class="img"
													src='<c:url value="/showImagePercentage/${percentages.percentageId}"/>'
...
<!-- 													src="http://pic.616pic.com/ys_bnew_img/00/04/81/RWvymh6QCe.jpg" /> -->
												...
												<form:input type="file" name="image" id="image"
													path="percentageImage" accept="image/*"
													onchange="loadFile(event)" />
											</div>
											<div>
												<label>活動時間:</label>
												<form:input path="percentageTime" id="datepicker"
													name="percentageTime" autocomplete="off" onblur="checkT()" />
											</div>
											<div>
												<form:radiobutton path="percentageOnOff" value="1" />
												開始活動
												<form:radiobutton path="percentageOnOff" value="0" />
												關閉活動
											</div>
											<div>
												<label>最低消費金額:</label>
												<form:input path="percentageMinimumCharge"
													id="percentageMinimumCharge" onblur="checkAMC()" />
												<span id="pmcsp" style="font-size: 10px;"></span>
											</div>
											<div>
												<label>折扣金額:</label>
												<form:input path="percentage" id="percentage"
													onblur="checkA()" />
												<span id="psp" style="font-size: 10px;"></span>
											</div>
											<span id="sbstr" style="font-size: 10px;"></span>
										<button type="submit" name="submit"
											onsubmit="return checkSubit()">修改</button>
										<a href="${contextRoot}/viewPercentages"><button type="button">返回</button></a>
										</form:form>
